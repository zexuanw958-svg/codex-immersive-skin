...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'Codex Immersive Skin: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-immersive-skin"
STATE_ROOT="$HOME/Library/Application Support/CodexImmersiveSkin"
STATE_PATH="$STATE_ROOT/state.json"
PREFERRED_PORT_PATH="$STATE_ROOT/preferred-port"
...
require_macos_runtime() {
  ...
  [ "$node_major" -ge 22 ] || fail "Codex bundled Node.js $NODE_VERSION is too old; version 22 or newer is required."
